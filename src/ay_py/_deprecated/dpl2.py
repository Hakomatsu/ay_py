...
FIXED:Merge TDynPlanLearn: the difference is only the TLWR.
Make Options['model_type'] of TDynPlanLearn2 available to switch TLWR and TNNRegression.
Go to base_ml_lwr and update TLWR so that TLWR inherits TFunctionApprox.
'''
...
class TDynPlanLearn2(object):
  ...
  def DefaultOptions():
    ...
    Options['base_dir']= '/tmp/dpl/'  #Base directory.  Last '/' is matter.
  ...
  def __init__(self, N, state_dims, action_bounds, assessments, ideal_funcs):
    assert(len(state_dims)==N+1)
  ...
  def Save(self, base_dir=None):
    ...
    data['params']= {}
    data['params']['state_dims']= ST(self.state_dims)
    ...
    if self.Options['model_type'] in ('lwr', 'dnn'):
      data['params']['models']= [{} for n in range(self.N)]
      for n in range(self.N):
        prefix= '{base}m_{n}_'.format(base='{base}', n=n)
        data['params']['models'][n]['prefix']= prefix
        data['params']['models'][n]['model']= prefix+file_suffix
        SaveYAML(self.models[n].Save(L(data['params']['models'][n]['prefix'])), L(data['params']['models'][n]['model']))
    return data
  ...
  def Load(self, data, base_dir=None):
    ...
    self.state_dims= data['params']['state_dims']
    ...
    if self.Options['model_type'] in ('lwr', 'dnn'):
      for n in range(self.N):
        self.models[n].Load(LoadYAML(L(data['params']['models'][n]['model'])), L(data['params']['models'][n]['prefix']))
  ...
  def Init(self):
    if self.models==None:  self.CreateModels()
    if self.Options['model_type'] in ('lwr', 'dnn'):
      for n in range(self.N):
        self.models[n].Options['base_dir']= self.Options['base_dir']
        self.models[n].Init()
  ...
  def CreateModels(self):
    ...
    if self.Options['model_type']=='lwr':
      ...
      options['base_dir']= self.Options['base_dir']
      for i in range(self.N):
        options['n_units']= [self.Dxa(i)] + list(self.Options['dnn_hidden_units']) + [self.Dx(i+1)]
        model= TNNRegression()
        model.Load(data={'options':options})
        self.models.append(model)
  ...
  class TEpisodeLog:
    def __init__(self, N):  #N: index of final stage; 0,..,N
      self.seq= [TDynPlanLearn2.TXAR() for n in range(N+1)]
  ...
  def Dx(self,n):
    return self.state_dims[n]

  def Da(self,n):
    if n==self.N:  return 0
    if self.action_bounds[n]==None:  return 0
    else:  return len(self.action_bounds[n][0])

  def Dxa(self,n):
    return self.Dx(n)+self.Da(n)
  ...
  def XABounds(self,n,f_x_expand=1.0):
    ...
    Dx,Da= self.Dx(n),self.Da(n)
    if n==self.N or self.action_bounds[n]==None:
  ...
  def IsPredictable(self, n):
    if self.models==None or self.N==0:  return False
    predictable= True
    for k in range(n,self.N):
      if self.models[k].NSamples<self.Options['num_min_predictable'] or not self.models[k].IsPredictable() or self.state_bounds[k]==None:
        predictable= False
        break
    return predictable
  ...
  def Update(self, n, x_n, xa_prev=None):
    ...
    '''
    if self.IsPredictable(1):
      #self.UpdateIdealsCMA(1)
      self.ModifyIdeals(1)
      for i in range(self.Options['ideals_grad_iter']):
        self.UpdateIdealsGrad(1)
      print 'self.ideals(x):'
      for k in range(1,self.N+1):  print '  %i: %s'%(k,[(xar.x,xar.R) for xar in self.ideals[k]])
    #'''
  ...
  def UpdateIdealsGrad(self, n):
    ...
    for i in range(len(self.ideals[n])):
      ...
      x_next= np.mat(pred.Y)
      F_xa_n= np.mat(pred.Grad).T  #Gradient of model
      F_x_n= F_xa_n[:,:Dx]
      F_a_n= F_xa_n[:,Dx:]
  ...
  def Select(self, n, x_n):
    if n==0:
      self.xar_log.append(self.TEpisodeLog(self.N))
      self.logid+= 1
    ...
    if self.action_bounds[n]==None:
      ...
      if not self.IsPredictable(n):
        ...
        #FIXME:TODO:This is better to be a function:
        if var_x_n==None:
          pass
        elif isinstance(var_x_n, (float, np.float_, np.float16, np.float32, np.float64)):
          var_x_n= np.diag([var_x_n]*x_n.size)
        else:
          if isinstance(var_x_n, (list,tuple)):  var_x_n= np.array(var_x_n)
          if var_x_n.size==x_n.size:
            var_x_n= np.diag(var_x_n.ravel())
          else:
            var_x_n= var_x_n.reshape(x_n.size,x_n.size)
        ...
        if self.Options['plan_method']=='grad':  a_map,eval_n= self.PlanGrad(n, x_n, var_x_n); a_n= a_map[n]
        elif self.Options['plan_method']=='sCMA':  a_map,eval_n= self.PlanSCMA(n, x_n, var_x_n); a_n= a_map[n]
        elif self.Options['plan_method']=='bfCMA':  a_n,eval_n= self.PlanBFCMA(n, x_n, var_x_n)
  ...
  def PlanBFCMA(self, n, x_n, var_x_n=None):
    if n>=self.N:  return None, self.Eval(n, x_n, None, var_x_n)
  ...
  def Eval(self, n, x_n, a_n, var_x_n=None):
    ...
    var_x_next= np.mat(pred.Var)*self.Options['plan_cautionary_ratio']
    #if var_x_n!=None:  var_x_next+= F_x_n * var_x_n * F_x_n.T  #FIXED: This should be unnecessary for DNN
  ...
  def EvalActMap(self, n, x_n, a_map, var_x_n=None, var_a_map={}):
    ...
    for k in range(n,self.N):
      ...
      var_x_next= np.mat(pred.Var)*self.Options['plan_cautionary_ratio']
      #if var_x_n!=None:  var_x_next+= F_x_n * var_x_n * F_x_n.T  #FIXED: This should be unnecessary for DNN
      #if k in var_a_map: var_x_next+= F_a_n * var_a_map[k] * F_a_n.T  #FIXED: This should be unnecessary for DNN
  ...
  def EvalIdealFunc(self, n, Dx, criteria):
    ...
    if n==self.N:
      if criteria=='sumR' or criteria=='R_xerr':
        fobj= lambda x_n: self.assessments[n](x_n)  #TODO:FIXME:Is it better to use plan_var_x?
    else:
      if criteria=='sumR':
        fobj= lambda xa_n: self.assessments[n](xa_n[:Dx]) + self.ideals[n+1][i_next].R
      elif criteria=='R_xerr':
        fobj= lambda xa_n: self.assessments[n](xa_n[:Dx]) - self.Options['reward_shape_f'] * la.norm(np.mat(self.ideals[n+1][i_next].x).T - np.mat(self.models[n].Predict(ToList(xa_n)).Y))**2
    return fobj
  ...
  def PlanSCMA(self, n, x_n, var_x_n=None):
    ...
    if n>=self.N:  return None, self.EvalActMap(n, x_n, none_a_map, var_x_n)
  ...
  def PlanGrad(self, n, x_n, var_x_n=None, tol=1.0e-4):
    ...
    for j in range(self.Options['grad_max_stages']):
      ...
      for i in range(Ni):
        a_map_new= {}
        grad_data= []  #For log
        self.StepGradOpt(n, x_n, var_x_n=var_x_n, a_map=a_map, a_map_new=a_map_new, grad_data=grad_data, criteria=criteria)
      ...
      if j>=Nj-1 and j<self.Options['grad_max_stages']-1:
        if self.Criteria(self.EvalActMap(n, x_n, a_map_best, var_x_n=var_x_n)) > self.Options['grad_term_quality_ratio']*R_ideals:
          break
  ...
  def InitGuess(self, n, x_n, var_x_n=None):
    if n>=self.N:
      ...
      #return {n:a_n}
    #else:
      #x_next= np.mat(self.models[n].Predict(ToList(x_n)+ToList(a_n)).Y)
  ...
  def StepGradOpt(self, n, x_n, var_x_n, a_map, a_map_new, criteria, with_DR=False, grad_data=None):
    ...
    F_a_n= F_xa_n[:,Len(x_n):]
    var_x_next= np.mat(pred.Var)*self.Options['plan_cautionary_ratio']
    #if var_x_n!=None:  var_x_next= F_x_n * var_x_n * F_x_n.T + np.mat(pred.Var)  #FIXED: This should be unnecessary for DNN
    #else:              var_x_next= np.mat(pred.Var)
    if n==self.N-1:
    ...
    if a_map[n]==None:
      ...
      #for i in range(1):
        ...
        #WARNING:FIXME: var_x_n=None should be reconsidered
  ...
  def PlotModel(self,n,f_reduce,f_repair,file_prefix='/tmp/f{n}',f_x_expand=None):
    ...
    DumpPlot(self.models[n], f_reduce=f_reduce, f_repair=f_repair, x_var=0.0, bounds=bounds, file_prefix=file_prefix.format(n=n))

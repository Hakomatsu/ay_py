...
class TMikata(object):
  ...
  def MoveTo(self, target, blocking=True, threshold=0.03):
    with self.port_locker:
      for jname,pos in list(target.items()):
  ...
  def MoveToC(self, target, blocking=True, threshold=0.03):
    with self.port_locker:
      for jname,(pos,curr) in list(target.items()):
  ...
  def SetCurrent(self, current):
    with self.port_locker:
      for jname,curr in list(current.items()):
  ...
  def SetVelocity(self, velocity):
    with self.port_locker:
      for jname,vel in list(velocity.items()):
  ...
  def SetPWM(self, pwm):
    with self.port_locker:
      for jname,pwm_ in list(pwm.items()):
